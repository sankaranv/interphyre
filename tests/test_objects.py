...
from interphyre.objects.base import PhyreObject
...
def test_ball_inherited_properties():
    ...
    assert ball.dynamic == True, "Ball should be dynamic by default"
...
def test_create_ball_function():
    ...
    assert body.userData == "test_ball", f"Expected userData='test_ball', got {body.userData}"
...
def test_create_ball_ccd_flag():
    ...
    assert body_without_ccd.bullet == False, "Body without CCD should have bullet=False"
    assert body_with_ccd.bullet == True, "Body with CCD should have bullet=True"


@pytest.mark.fast
def test_ball_property_access():
    """Test accessing all Ball properties."""
    ball = Ball(x=1.0, y=2.0, radius=0.8, color="blue", 
                friction=0.6, restitution=0.4, density=1.5, dynamic=False)
...
def test_bar_ramp_to_wall_left():
    ...
    try:
        bar = Bar.ramp_to_wall(start_x=0, start_y=0, angle=135, wall_side="left", thickness=0.2)
...
def test_bar_ramp_to_wall_right():
    ...
    bar = Bar.ramp_to_wall(start_x=0, start_y=0, angle=0, wall_side="right", thickness=0.2)
...
def test_bar_ramp_to_wall_top():
    ...
    bar = Bar.ramp_to_wall(start_x=0, start_y=0, angle=90, wall_side="top", thickness=0.2)
...
def test_bar_ramp_to_wall_bottom():
    ...
    # We test that it doesn't crash and creates a valid Bar object
    try:
        bar = Bar.ramp_to_wall(start_x=0, start_y=0, angle=225, wall_side="bottom", thickness=0.2)
        # Verify bar is created
        assert isinstance(bar, Bar), "Should create a Bar object"
        assert bar.thickness == 0.2, "Bar should have correct thickness"
    except (ZeroDivisionError, ValueError):
...
def test_bar_ramp_to_wall_invalid_side():
    ...
    with pytest.raises(ValueError, match="Invalid wall_side"):
        Bar.ramp_to_wall(start_x=0, start_y=0, angle=0, wall_side="invalid", thickness=0.2)
...
def test_bar_offset_along_angle():
    ...
    bar = Bar.offset_along_angle(base_x=0, base_y=0, angle=45.0, distance=2.0, thickness=0.2)
...
def test_basket_from_width_and_flare():
    ...
    assert abs(basket.bottom_width - 2.0) < 1e-9
    assert abs(basket.top_width - 2.4) < 1e-9, f"Expected top_width=2.4, got {basket.top_width}"
...
def test_basket_anchor_bottom_center():
    ...
    basket = Basket(x=0, y=-3, bottom_width=2.0, top_width=2.4, height=2.5, anchor="bottom_center")
...
def test_basket_anchor_center():
    ...
    basket = Basket(x=0, y=-3, bottom_width=2.0, top_width=2.4, height=2.5, anchor="center")
    offset = basket.get_anchor_offset()
...
def test_basket_anchor_top_center():
    ...
    basket = Basket(x=0, y=-3, bottom_width=2.0, top_width=2.4, height=2.5, anchor="top_center")
    offset = basket.get_anchor_offset()
...
def test_basket_anchor_bottom_left():
    ...
    basket = Basket(x=0, y=-3, bottom_width=2.0, top_width=2.4, height=2.5, anchor="bottom_left")
    offset = basket.get_anchor_offset()
...
def test_basket_anchor_bottom_right():
    ...
    basket = Basket(x=0, y=-3, bottom_width=2.0, top_width=2.4, height=2.5, anchor="bottom_right")
    offset = basket.get_anchor_offset()
    expected_x = -basket.bottom_width / 2
    assert abs(offset[0] - expected_x) < 1e-9 and abs(offset[1] - 0) < 1e-9, \
        f"Expected offset=({expected_x}, 0), got {offset}"
...
def test_basket_anchor_top_left():
    ...
    basket = Basket(x=0, y=-3, bottom_width=2.0, top_width=2.4, height=2.5, anchor="top_left")
    offset = basket.get_anchor_offset()
...
def test_basket_anchor_top_right():
    ...
    basket = Basket(x=0, y=-3, bottom_width=2.0, top_width=2.4, height=2.5, anchor="top_right")
    offset = basket.get_anchor_offset()
    expected_x = -basket.top_width / 2
    expected_y = -basket.height
    assert abs(offset[0] - expected_x) < 1e-9 and abs(offset[1] - expected_y) < 1e-9, \
...
def test_basket_wall_thickness():
    ...
    basket = Basket(x=0, y=-3, bottom_width=2.0, wall_thickness=0.3, floor_thickness=0.4)
...
def test_basket_sensor_enabled():
    ...
    assert basket.enable_sensor == True
    
    basket_no_sensor = Basket(x=0, y=-3, bottom_width=2.0, enable_sensor=False)
    assert basket_no_sensor.enable_sensor == False
...
def test_basket_double_walls():
    ...
    assert basket.double_walls == True
...
def test_basket_segmented_walls():
    ...
    assert basket.segmented_walls == True
...
def test_create_bar_precision_rounding():
    ...
    world = b2World(gravity=(0, -10))
    bar = Bar(x=1.123456789, y=2.987654321, length=4.555555555, angle=45.123456789, thickness=0.2)
    body = create_bar(world, bar, "test_bar")
...
def test_create_basket_fixture_count():
    ...
    world = b2World(gravity=(0, -10))
    basket = Basket(x=0, y=-3, bottom_width=2.0, top_width=2.4, height=2.5, enable_sensor=False)
...
def test_create_basket_double_walls_extra_fixtures():
    ...
    assert len(body.fixtures) == 5, f"Expected 5 fixtures with double_walls, got {len(body.fixtures)}"
...
def test_create_basket_dynamic_vs_static():
    ...
    dynamic_basket = Basket(x=0, y=-3, bottom_width=2.0, dynamic=True, enable_sensor=False)
    static_basket = Basket(x=1, y=-3, bottom_width=2.0, dynamic=False, enable_sensor=False)
...
def test_create_basket_anchor_positioning():
    ...
    basket1 = Basket(x=0, y=-3, bottom_width=2.0, anchor="bottom_center", enable_sensor=False)
