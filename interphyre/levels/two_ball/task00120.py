...
def build_level(seed=None, variant=0, scene=None) -> Level:
    ...
    y_positions = np.linspace(
        0.15,
        (ball_top - 2 * ball_radius - MIN_Y) / WORLD_HEIGHT,
        bar_count,
    )
    for idx, y_frac in enumerate(reversed(y_positions)):
        ...
        bar_y = MIN_Y + (y_frac + rng.normal() * 0.015) * WORLD_HEIGHT
        bar = Bar(
    ...
    lower_bar = Bar(
        ...
        y=MIN_Y + 0.12 * WORLD_HEIGHT + bar_thickness / 2,
        thickness=bar_thickness,
        color="black",
        dynamic=False,
    )
    ...
    pair_y = MIN_Y + 0.25 * WORLD_HEIGHT
    pair_top = Bar(
    ...
    pair_bottom = Bar(
        ...
        y=pair_y - 0.07 * WORLD_HEIGHT + bar_thickness / 2,
        thickness=bar_thickness,
        color="black",
        dynamic=False,
    )
