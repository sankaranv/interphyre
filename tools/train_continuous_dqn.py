#!/usr/bin/env python3
"""
Continuous DQN training script for Interphyre.

This script trains a continuous DQN agent on the down_to_earth level
with better reward shaping and continuous actions.
"""
...
def train_continuous_dqn(
    level_name: str = "down_to_earth",
    num_episodes: int = 200,
    max_trials: int = 15,
    save_path: str = "continuous_dqn_model.pth",
    eval_interval: int = 20,
):
    """
    Train a continuous DQN agent on a level.
    ...
    print(f"Training Continuous DQN agent on {level_name} level...")
    print(f"Episodes: {num_episodes}")
    print(f"Max trials per episode: {max_trials}")
    print()

    # Create continuous DQN agent with better hyperparameters
    agent = ContinuousDQNAgent(
        name="continuous_dqn_trained",
        seed=42,
        ...
        noise_std=0.2,  # More exploration noise
    )
    ...
    for episode in range(num_episodes):
        ...
        if (episode + 1) % eval_interval == 0:
            ...
            eval_stats_batch = []
            for eval_ep in range(10):  # More evaluation episodes
            ...
            if eval_success_rate > best_success_rate:
                ...
                print(f"New best model saved! Success rate: {eval_success_rate:.1%}")
    ...
    final_eval_stats = []
    for eval_ep in range(20):  # More final evaluation episodes
...
def main():
    ...
    results = train_continuous_dqn(
        level_name="down_to_earth",
        num_episodes=100,  # More episodes for better learning
        max_trials=15,
        save_path="models/continuous_dqn_down_to_earth.pth",
        eval_interval=20,
    )

    # Save results
    os.makedirs("results", exist_ok=True)
    with open("results/continuous_dqn_training_results.json", "w") as f:
    ...
    print(f"\nResults saved to results/continuous_dqn_training_results.json")
