...
def merge_hdf5_shards(
    ...
    with h5py.File(str(output_path), "w", libver="latest") as out:
        ...
        n_total = len(all_ids)
        logger.info("Total instances: %d", n_total)
        ...
        for sf in shard_files:
            with h5py.File(str(sf), "r") as f:
                ...
                if has_embed and "embed" in f:
                    out["embed"][write_idx:write_idx + n_shard] = f["embed"][:]
                write_idx += n_shard

        # Copy stats from the first shard that has them.
        for sf in shard_files:
            with h5py.File(str(sf), "r") as f:
                if "stats" in f:
                    f.copy("stats", out)
...
def main() -> None:
    ...
    parser.add_argument("--levels", nargs="+", default=["down_to_earth", "end_of_line", "two_body_problem"])
