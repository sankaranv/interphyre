#!/bin/bash
#SBATCH --job-name=bundle_staircase_v3
#SBATCH --partition=cpu-preempt
#SBATCH --account=pi_jensen_umass_edu
#SBATCH --cpus-per-task=16
#SBATCH --mem=16G
#SBATCH --time=04:00:00
#SBATCH --output=/scratch4/workspace/svaidyanatha_umass_edu-phyre/logs/bundle_staircase_v3_%j.out
#SBATCH --error=/scratch4/workspace/svaidyanatha_umass_edu-phyre/logs/bundle_staircase_v3_%j.err

# Bundle regen for staircase with k=25 (up from k=10).
...
# model(k=10)=147.6 impossible; model(k=25)=0.3 impossible.
# Expected: ~100% valid.
...
echo "[bundle_staircase_v3] Starting at $(date)"

python -u -m interphyre.validation._bundle \
    --levels staircase \
    --seeds 0:10001 \
    --workers 16 \
    --attempts 500 --max-variants 25

echo "[bundle_staircase_v3] Done at $(date)"
