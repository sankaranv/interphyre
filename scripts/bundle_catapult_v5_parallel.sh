...
# v5 (2026-04-14): arm_right constraint [-2.0,-1.5], ledge_y [-4,-2.5], Zone A x bias: oracle-steps 500→1000 (= config.max_steps); attempts 200→500
#   per register_defaults. Full catapult trajectory requires ≥1000 steps.

set -euo pipefail
PROJECT=/work/pi_jensen_umass_edu/svaidyanatha_umass_edu/interphyre
TMPDIR=/scratch4/workspace/svaidyanatha_umass_edu-phyre/catapult_v5_chunks
mkdir -p "$TMPDIR"

submit_chunk() {
    local chunk=$1 start=$2 stop=$3
    sbatch --parsable \
        --job-name="catapult_v5_chunk_${chunk}" \
        --partition=cpu-preempt \
        --account=pi_jensen_umass_edu \
        --cpus-per-task=16 \
        --mem=16G \
        --time=02:00:00 \
        --output="/scratch4/workspace/svaidyanatha_umass_edu-phyre/logs/catapult_v5_chunk_${chunk}_%j.out" \
        --error="/scratch4/workspace/svaidyanatha_umass_edu-phyre/logs/catapult_v5_chunk_${chunk}_%j.err" \
        --wrap="
            ...
            python -u -m interphyre.validation._bundle \
                ...
                --attempts 500 \
                --oracle-steps 1000 \
                --output $TMPDIR/catapult_v5_chunk_${chunk}.json.lzma
...
MERGE_JOB=$(sbatch --parsable \
    ...
    --output="/scratch4/workspace/svaidyanatha_umass_edu-phyre/logs/catapult_v5_merge_%j.out" \
    --error="/scratch4/workspace/svaidyanatha_umass_edu-phyre/logs/catapult_v5_merge_%j.err" \
    --wrap="
        . $PROJECT/.venv/bin/activate
        python -u -c \"
import lzma, json
...
chunks = sorted(chunk_dir.glob('catapult_v5_chunk_*.json.lzma'))
