#!/bin/bash
#SBATCH --job-name=oracle_val_cradle
#SBATCH --partition=cpu-preempt
#SBATCH --account=pi_jensen_umass_edu
#SBATCH --cpus-per-task=8
#SBATCH --mem=8G
#SBATCH --time=02:00:00
#SBATCH --output=/scratch4/workspace/svaidyanatha_umass_edu-phyre/logs/oracle_val_cradle_%j.out
#SBATCH --error=/scratch4/workspace/svaidyanatha_umass_edu-phyre/logs/oracle_val_cradle_%j.err

# Validates Gaussian y oracle improvement for the_cradle.
# Tests on seeds 0:500 and measures avg_var vs baseline (1.725).
# Zone A y changed from uniform(2.5, 4.5) to Gaussian(3.85, 0.5) clipped to [2.5, 4.5].
# Decision criterion: avg_var drop >= 15% (to < 1.47) -> trigger full bundle regen.

set -euo pipefail
PROJECT=/work/pi_jensen_umass_edu/svaidyanatha_umass_edu/interphyre
source $PROJECT/.venv/bin/activate
echo "[oracle_val_cradle] Starting at $(date)"

OUTDIR=/scratch4/workspace/svaidyanatha_umass_edu-phyre/logs
OUTFILE=$OUTDIR/cradle_val_${SLURM_JOB_ID}.json.lzma
...
echo "[oracle_val_cradle] Bundle done at $(date)"
...
print(f'the_cradle oracle validation (500 seeds):')
