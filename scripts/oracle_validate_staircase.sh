#!/bin/bash
#SBATCH --job-name=oracle_val_staircase
#SBATCH --partition=cpu-preempt
#SBATCH --account=pi_jensen_umass_edu
#SBATCH --cpus-per-task=8
#SBATCH --mem=8G
#SBATCH --time=02:00:00
#SBATCH --output=/scratch4/workspace/svaidyanatha_umass_edu-phyre/logs/oracle_val_staircase_%j.out
#SBATCH --error=/scratch4/workspace/svaidyanatha_umass_edu-phyre/logs/oracle_val_staircase_%j.err

# Validates two-Gaussian x oracle improvement for staircase.
# Tests on seeds 0:500 and measures avg_var vs baseline (1.957).
# Decision criterion: avg_var drop >= 15% (to < 1.66) -> trigger full bundle regen.

set -euo pipefail
PROJECT=/work/pi_jensen_umass_edu/svaidyanatha_umass_edu/interphyre
source $PROJECT/.venv/bin/activate
echo "[oracle_val_staircase] Starting at $(date)"

OUTDIR=/scratch4/workspace/svaidyanatha_umass_edu-phyre/logs
OUTFILE=$OUTDIR/staircase_val_${SLURM_JOB_ID}.json.lzma
...
echo "[oracle_val_staircase] Bundle done at $(date)"
...
print(f'staircase oracle validation (500 seeds):')
